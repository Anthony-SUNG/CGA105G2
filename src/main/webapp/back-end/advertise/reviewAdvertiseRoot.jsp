            ...
            <div class="table-responsive ">
                <table class="table table-striped text-center ">
                    ...
                    <tr class="col-3">
                        <td>${ad.advId} </td>
                        <td>${ad.storeId} </td>
                        <td><img src="<%=request.getContextPath()%>/adServlet?action=getPhoto&adId=${ad.advId}" style="max-width:60px;max-height:45px"></td>
                        <td>${ad.advStatus} </td>
                        <td>${ad.advText} </td>
                        <td>${ad.advTime} </td>
                        <td>${ad.advStime} </td>
                        <td>${ad.advNtime} </td>
                        <td>
                            <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/adServlet" style="margin-bottom: 0;">
                                <select size="1" name="empId" id="selectEmp">
                                    <c:forEach var="emp" items="${empSvc.rootEmpBy}">
                                        <c:if test="${emp.rootId!=1}">
                                            <option value="${emp.emp.empId}">${emp.emp.empAcc}</option>
                                        </c:if>
                                    </c:forEach>
                                </select>
