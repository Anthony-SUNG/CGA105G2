...
public class StandbyServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		System.out.println("sts:" + onOff);
		req.setAttribute("onOff", onOff);
		StandbyService staSvc = new StandbyService();

		if ("getStoreSts".equals(action)) {
			res.setContentType("text/plain");
            res.getWriter().write(onOff);
			// if (onOff.equals("on")) {
			// 	res.getWriter().write(onOff);
			// } else {
			// 	res.getWriter().write("off");
			// }
		}

        if("getStaConut".equals(action)){
            res.setContentType("text/plain");
            
            Integer staCount = staSvc.standByCount();
            System.out.println(staCount);
            res.getWriter().write(staCount);
		...
		if ("insertSta".equals(action)) {
            ...
            System.out.println(standbyVo.getStaId());
			req.setAttribute("standbyVo", standbyVo);
