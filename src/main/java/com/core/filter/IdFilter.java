...
//		request.getServletPath() 方法獲取請求的服務路徑。這個方法返回請求的路徑，不包括上下文路徑和查詢字符串。例如，如果請求的 URL 是 "http://example.com/myapp/hello"，則該方法將返回 "/hello"。
        ...
        logger.info(String.format("WEB INFO[ServerName:%s,requestCon:%s,requestPath:%s,info:%s", ServerName, requestCon, requestPath, info));
...
//		response.addCookie(cookie);
        if (requestPath.matches(".*css") || requestPath.matches(".*js") || requestPath.matches(".*assets.*")) {
            ...
            chain.doFilter(request, response);
                ...
                requestPath.matches(".*contactUs.jsp.*") || requestPath.matches(".*Loader.*") || requestPath.matches(".*SendMailServlet.*") ||
                requestPath.matches(".*FriendWS.*") || requestPath.matches(".*open.*")
        ) {
            if (requestPath.equals("/adServlet")) {
                logger.info("doFilter-2區:廣告區");
                chain.doFilter(request, response);
                return;
            }
            logger.info("doFilter-3區:免登入區");
            if (mId >= 1) {
                chain.doFilter(request, response);
                return;
            }
            ...
            logger.info("doFilter-4區:Servlet跳轉區");
            chain.doFilter(request, response);
        } else if (sId >= 1 && (storeplan == 0 || storeplan == null)) {
            ...
            logger.info("doFilter-8區:後台區");
            chain.doFilter(request, response);
        } else if (requestPath.matches(".*/CGA105G2/")) {
            logger.info("doFilter-9區:首頁區");
            chain.doFilter(request, response);
        } else {
